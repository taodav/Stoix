        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    # Continuous (tanh-squashed Normal) entropy is estimated by
                    # sampling, so it needs a PRNG seed.
                ...
                actor_grads, actor_loss_info = actor_grad_fn(
                    params.actor_params, traj_batch, advantages, actor_loss_key
                )
            ...
            key, shuffle_key, minibatch_key = jax.random.split(key, 3)
            ...
            (params, opt_states, _), loss_info = jax.lax.scan(
                _update_minibatch, (params, opt_states, minibatch_key), minibatches
            )
...
def learner_setup(
    ...
    # Get number/dimension of actions. Continuous control: the action space is a
    # box, so use its trailing shape and record its bounds for the tanh head.
...
@hydra.main(
    ...
    config_name="default_rec_ppo_nonrec_actor_continuous.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(
        f"{Fore.CYAN}{Style.BRIGHT}Non-recurrent actor + dual-value (continuous) experiment completed"
        f"{Style.RESET_ALL}"
    )
    return eval_performance
