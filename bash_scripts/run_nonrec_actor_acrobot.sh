...
#
# Uses ACTION CONCATENATION (env=gymnax/acrobot_action_concat): the previous action
# is appended to each observation. Only the RECURRENT critic sees it; the reactive
# actor and the memory-free critic strip it (they stay Markov) -- so the reactive
# actor's Markov property is preserved even with action-concat on.
#
# Checkpoints: ./checkpoints/rec_ppo_nonrec_actor/acrobot_nonrec_actor_ac_h64/
...
"${PYTHON}" stoix/systems/ppo/anakin/rec_ppo_nonrec_actor.py \
  ...
  logger.checkpointing.save_args.checkpoint_uid=acrobot_nonrec_actor_ac_h64 \
